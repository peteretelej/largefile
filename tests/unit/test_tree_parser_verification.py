...
'''
        ...
        root = tree.root_node
        assert root.type == 'module', f"Python AST root should be 'module', got '{root.type}'"
...
'''
        ...
        assert any("function_one" in name for name in outline_names), "Should find function_one"
        assert any("MyClass" in name for name in outline_names), "Should find MyClass"
        assert any("method_one" in name for name in outline_names), "Should find method_one"
        assert any("function_two" in name for name in outline_names), "Should find function_two"
        ...
        for item in outline:
            assert item.line_number > 0, f"Line numbers should be positive, got {item.line_number}"
            assert item.line_number <= 15, f"Line numbers should be within content range"
...
'''
        ...
        has_nested_structure = any(len(item.children) > 0 for item in outline)
