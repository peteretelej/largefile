...
class TestFileAccessStrategies:
    ...
    def test_mmap_strategy_with_shakespeare(self):
        ...
        with override_env(LARGEFILE_MEMORY_THRESHOLD_MB="1"):
            ...
            assert file_info["strategy"] == "mmap", f"Expected mmap strategy, got {file_info['strategy']}"
            ...
            search_results = search_content(str(shakespeare_file), "love", max_results=5)
            assert search_results["total_matches"] > 0, "Should find 'love' in Shakespeare"
    ...
    def test_streaming_strategy_with_shakespeare(self):
        ...
        with override_env(LARGEFILE_MEMORY_THRESHOLD_MB="1", LARGEFILE_MMAP_THRESHOLD_MB="2"):
            # Verify strategy selection
            file_info = get_file_info(str(shakespeare_file))
            assert file_info["strategy"] == "streaming", f"Expected streaming strategy, got {file_info['strategy']}"
    ...
    def test_streaming_strategy_with_lodash(self):
        ...
        with override_env(LARGEFILE_MEMORY_THRESHOLD_MB="0", LARGEFILE_MMAP_THRESHOLD_MB="0"):
            # Verify strategy selection  
    ...
    def test_mmap_vs_streaming_boundary(self):
        ...
        shakespeare_file = self.test_data_dir / "text" / "shakespeare-complete.txt"  # 5.36MB
        
        if not shakespeare_file.exists():
            return
        
        # Force mmap by setting thresholds: memory=1MB, mmap=10MB  
        with override_env(LARGEFILE_MEMORY_THRESHOLD_MB="1", LARGEFILE_MMAP_THRESHOLD_MB="10"):
            file_info = get_file_info(str(shakespeare_file))
    ...
    def test_strategy_error_handling(self):
        ...
        for memory_mb, mmap_mb in [("50", "500"), ("1", "500"), ("1", "2")]:
            with override_env(LARGEFILE_MEMORY_THRESHOLD_MB=memory_mb, LARGEFILE_MMAP_THRESHOLD_MB=mmap_mb):
                try:
                    get_file_info(nonexistent)
                    assert False, "Should have raised FileAccessError"
    ...
    def test_strategy_consistency(self):
        ...
        with override_env(LARGEFILE_MEMORY_THRESHOLD_MB="1"):
            ...
            assert len(set(strategies)) == 1, f"Strategy selection not consistent: {strategies}"
            assert strategies[0] == "mmap", f"Expected mmap strategy, got {strategies[0]}"