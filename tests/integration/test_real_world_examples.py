...
from src.tools import get_overview, search_content, read_content
...
class TestRealWorldExamples:
    ...
    def test_ai_documentation_assistant(self):
        ...
        for doc in ai_docs:
            ...
            async_results = search_content(str(doc), "async", fuzzy=True, context_lines=2)
    ...
    def test_code_migration_helper(self):
        ...
        if laravel_file.exists():
            ...
            laravel_methods = search_content(str(laravel_file), "function", max_results=10)
    ...
    def test_technical_specification_navigator(self):
        ...
        connection_results = search_content(str(rfc_file), "connection", fuzzy=True, max_results=10)
        http_results = search_content(str(rfc_file), "HTTP", fuzzy=False, max_results=5)
        
        # Should find relevant specification content
        total_matches = connection_results["total_matches"] + http_results["total_matches"]
    ...
    def test_polyglot_development_assistant(self):
        ...
        for lang, file_path in polyglot_files:
            ...
            function_patterns = search_content(str(file_path), "function", fuzzy=True, max_results=3)
            error_patterns = search_content(str(file_path), "error", fuzzy=True, max_results=3)
    ...
    def test_large_codebase_exploration(self):
        ...
        utility_functions = [
            "map", "filter", "reduce", "forEach", "find", "each"
        ]
    ...
    def test_documentation_authoring_assistant(self):
        ...
        for doc in docs:
            ...
            install_info = search_content(str(doc), "install", fuzzy=True, max_results=3)
            usage_info = search_content(str(doc), "usage", fuzzy=True, max_results=3)
            example_info = search_content(str(doc), "example", fuzzy=True, max_results=3)
